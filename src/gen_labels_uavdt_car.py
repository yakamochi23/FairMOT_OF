...
def gen_track_dataset(src_root, dst_root, viz_root=None):
    ...
    for seq in tqdm(seq_names):
        ...
        # ignore領域を取得---------------------------------------------------------------------------------------
        seq_ignore_f_path = src_root + '/annotations/' + seq + '_gt_ignore.txt'
        if not os.path.isfile(seq_ignore_f_path):
            print('[Warning]: invalid invalid annotations ignore file(txt).')
            continue
        
        # ignoreファイルを読み込み、ラベル情報を2次元配列に保存
        with open(seq_ignore_f_path, 'r', encoding='utf-8') as f_r:
            label_lines = f_r.readlines()
            label_n_lines = len(label_lines)
            seq_ignore_box_label = np.zeros((label_n_lines, 9), np.int32)

            # 各行を解析し、配列に格納
            for line_i, line in enumerate(label_lines):
                line = [int(x) for x in line.strip().split(',')]
                seq_ignore_box_label[line_i] = line
        # ------------------------------------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    #gen_track_dataset(src_root='D:/Dataset/UAVDT_mot/train',
    #                  dst_root='D:/Dataset/UAVDT_car/images/train',
    #                  viz_root='D:/Dataset/UAVDT_car/viz/train')
    
    #gen_track_dataset(src_root='D:/Dataset/UAVDT_mot/test',
    #                  dst_root='D:/Dataset/UAVDT_car/images/test',
    #                  viz_root='D:/Dataset/UAVDT_car/viz/test')
    
    # trainタスクの.trainファイルを生成
    gen_dot_train_file(
        data_root = 'D:/Dataset/', # データセットのルートディレクトリ
        rel_path='UAVDT_car/images/train', # 相対パス
        out_root='C:/Users/Image--lab/Desktop/FairMOT/src/data', # 出力先ディレクトリ
        f_name='uavdt_car.train' # 出力ファイル名
    )
