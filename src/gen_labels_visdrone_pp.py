...
def gen_track_dataset(src_root, dst_root, viz_root=None):
    ...
    dst_img_root = dst_root
    dst_txt_root = os.path.join(dst_root.replace('images', 'labels_with_ids'))
    ...
    for seq in tqdm(seq_names):
        ...
        dst_seq_img_dir = dst_img_root + '/' + seq + '/img1'
        if not os.path.isdir(dst_seq_img_dir):
            os.makedirs(dst_seq_img_dir)
        #dst_seq_txt_dir = dst_txt_root + '/' + seq
        dst_seq_txt_dir = dst_txt_root + '/' + seq + '/img1'
        if not os.path.isdir(dst_seq_txt_dir):
            os.makedirs(dst_seq_txt_dir)

        # gtファイルのフィルタリング--------------------------------------------------------------------------------
        dst_gt_txt_dir = dst_img_root + '/' + seq + '/gt'
        if not os.path.isdir(dst_gt_txt_dir):
            os.makedirs(dst_gt_txt_dir)

        dst_gt_txt_path = dst_gt_txt_dir + '/gt.txt'

        with open(seq_txt_f_path, 'r') as srcfile, open(dst_gt_txt_path, 'w') as dstfile:
            for line in srcfile:
                columns = line.strip().split(',')
                if len(columns) >= 7 and columns[7] in ['1', '2']:
                    # 7番目の列が2の場合、1に変更
                    if columns[7] == '2':
                        columns[7] = '1'

                    # 修正した行を書き込み
                    dstfile.write(','.join(columns) + '\n')
        # --------------------------------------------------------------------------------------------------------        
...
def gen_gt_file(src_root, dst_root):
    #    src_root='D:/Dataset/VisDrone2019_mot/train',
    #    dst_root='D:/Dataset/VisDrone_pedi/images/train'
...
if __name__ == '__main__':
    ...
    gen_track_dataset(
        src_root='D:/Dataset/VisDrone2019_mot/train',
        dst_root='D:/Dataset/VisDrone_pp/images/train',
        viz_root='D:/Dataset/VisDrone_pp/viz/train'
    )

    gen_track_dataset(
        src_root='D:/Dataset/VisDrone2019_mot/val',
        dst_root='D:/Dataset/VisDrone_pp/images/val',
        viz_root='D:/Dataset/VisDrone_pp/viz/val'
    )

    gen_track_dataset(
        src_root='D:/Dataset/VisDrone2019_mot/test',
        dst_root='D:/Dataset/VisDrone_pp/images/test',
        viz_root='D:/Dataset/VisDrone_pp/viz/test'
    )

    # trainタスクの.trainvalファイルを生成
    #gen_dot_train_file(
    #    data_root = 'D:/Dataset/', # データセットのルートディレクトリ
    #    rel_path='VisDrone_pedi/images/trainval', # 相対パス
    #    out_root='C:/Users/Image--lab/Desktop/FairMOT/src/data', # 出力先ディレクトリ
    #    f_name='visdrone_pedi.trainval' # 出力ファイル名
    #)

    #gen_gt_file(
    #    src_root='D:/Dataset/VisDrone2019_mot/test',
    #    dst_root='D:/Dataset/VisDrone_pedi/images/test'
    #)